...
def generate_for_online_dpo(
    ...
    **kwargs,
    ...
    for response in stream_generate(model, tokenizer, prompt, **kwargs):
        logprobs = mx.concat(response.logprobs, axis=0)
        text += response.text
    return logprobs, text

def get_scores(scores: List = None):
    return [1.22, 0.534]
...
def dpo_loss(
    pi_logratios: mx.array,
    ref_logratios: mx.array,
    policy_logprobs: mx.array,
    policy_rejected_logprobs: mx.array,
    scores: mx.array,
    ...
    kl = policy_logprobs - policy_rejected_logprobs
    non_score_reward = (-beta * kl).sum(1)
    margin = chosen_rewards - gathered_rejected_rewards

    # Metrics
    metrics = {
        "kl": mx.mean(kl.sum(1)),
        "accuracies": mx.mean((margin > 0).astype(mx.float32)),
        "rejected": mx.mean(policy_rejected_score / num_rejected_tokens),
        "chosen": mx.mean(policy_chosen_score / num_chosen_tokens),
        "margins": mx.mean(margin),
        "accuracies": mx.mean(accuracy),
        "entropy": mx.mean(-logprobs.sum(1)),
        "reward": mx.mean(reward),
...
def train_dpo(
    ...
    def step(batch):
        ...
        answer_a_logprobs, answer_a_text = generate_for_online_dpo(model, tokenizer, prompts)
        answer_b_logprobs, answer_b_text = generate_for_online_dpo(model, tokenizer, prompts)
